...
const PrescriptionsPage: React.FC = () => {
  ...
  const [itemToDelete, setItemToDelete] = useState<PrescriptionData | null>(
    null
  );
  ...
  const createMutation = useMutation({
    ...
    onError: (error: unknown) => {
      // ✅ FIX 1A: Change 'any' to 'unknown'
      // Assert type for safer access
    ...
    <div className="pr-[5.5rem] p-6">
      ...
      {fetchError && (
        <Alert variant="destructive" className="mb-4">
          ...
          <AlertDescription>
            {(fetchError as Error).message || "Could not load data."}
          </AlertDescription>
        </Alert>
      )}
      {/* Empty State */}
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle className="text-lg">
                      Prescription for{" "}
                      {p.patient.full_name || p.patient.username}
                    </CardTitle>
                    <CardDescription>
                  ...
                  <Button
                    ...
                    disabled={
                      deleteMutation.isPending && itemToDelete?.id === p.id
                    }
                  >
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-medium">
                {" "}
                {itemToDelete?.patient.full_name}
              </span>{" "}
              (Ref: #{itemToDelete?.id}). This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
const PrescriptionForm: React.FC<{
  ...
  const [medications, setMedications] = useState<Omit<MedicationData, "id">[]>([
    { name: "", dosage: "", frequency: "", instructions: "" },
  ]);

  const handleMedicationChange = (
    index: number,
    field: keyof Omit<MedicationData, "id">,
    value: string
  ) => {
    const newMeds = [...medications];
    (newMeds[index] as Omit<MedicationData,'id'>)[field] = value;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!diagnosis.trim()) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Diagnosis cannot be empty.",
      });
      return;
    }
    if (
      ...
      toast({
        ...
        title: "Error",
        description:
          "Please ensure all medications have a name, dosage, and frequency.",
      });
      return;
    }
  ...
  return (
    ...
    >
      ...
      {medications.map((med, index) => (
        <Card
          key={index}
          className="border p-4 pt-8 rounded-md space-y-3 relative shadow-sm"
        >
