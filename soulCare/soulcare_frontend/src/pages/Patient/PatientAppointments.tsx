...
import { Calendar, Clock, Plus, User, X, Trash2, Star, MessageSquare, CheckCircle2 } from "lucide-react";
...
const RateDialog: React.FC<RateDialogProps> = ({ isOpen, onClose, appointment }) => {
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[425px] rounded-xl shadow-2xl">
                <DialogHeader className="text-center sm:text-left">
                    <DialogTitle className="text-2xl font-semibold">Rate Your Experience</DialogTitle>
                    <DialogDescription>
                        How was your appointment with <span className="font-medium text-foreground">{appointment.provider.profile?.full_name || appointment.provider.username}</span>?
                    </DialogDescription>
                </DialogHeader>
                <div className="grid gap-6 py-6">
                    ...
                    <div className="grid gap-2">
                        <Label htmlFor="comment" className="font-medium">Additional Feedback</Label>
                        <Textarea
                            id="comment"
...
const PatientAppointments: React.FC = () => {
  ...
  const AppointmentCard = ({ appointment, showCancel, showDelete, showRate }: { appointment: Appointment, showCancel?: boolean, showDelete?: boolean, showRate?: boolean }) => (
    <Card key={appointment.id} className="group border border-border/50 bg-card hover:bg-card/50 transition-all duration-200 hover:shadow-md hover:border-primary/20 rounded-xl overflow-hidden">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
            <Avatar className="w-16 h-16 border-2 border-background shadow-sm group-hover:border-primary/20 transition-colors">
                <AvatarImage src={(appointment.provider.profile as any).profile_picture_url} className="object-cover" />
                <AvatarFallback className="bg-primary/5 text-primary font-semibold text-lg">
                    {(appointment.provider.profile?.full_name || appointment.provider.username)[0].toUpperCase()}
                </AvatarFallback>
            </Avatar>
            <div className="space-y-1.5">
                <h3 className="font-bold text-lg text-foreground tracking-tight leading-none">
                    {appointment.provider.profile.full_name || appointment.provider.username}
                </h3>
            ...
            <div className="flex items-center gap-2">
                ...
                {showRate && appointment.status === 'completed' && (
                    <Button 
                        ...
                        onClick={() => !appointment.has_review && openRateDialog(appointment)}
                ...
                <AlertDialog>
                    ...
                    <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive hover:bg-destructive/10 font-medium" disabled={cancelMutation.isPending}>
                    ...
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                        Are you sure? This will free up the slot for other patients.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel className="rounded-lg">Keep Appointment</AlertDialogCancel>
                        <AlertDialogAction className="bg-destructive hover:bg-destructive/90 rounded-lg" onClick={() => handleCancel(appointment.id)}>
                        Yes, Cancel
                        </AlertDialogAction>
                    </AlertDialogFooter>
                    </AlertDialogContent>
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pb-6 border-b">
          <div>
            ...
            <p className="text-muted-foreground mt-2 text-lg">Manage your bookings and view your medical history.</p>
          </div>
          <Button asChild className="shadow-lg hover:shadow-primary/25 transition-all rounded-full px-6 h-12 text-base">
            ...
            <Tabs defaultValue="scheduled" className="w-full">
                <TabsList className="grid w-full grid-cols-3 mb-8 p-1 bg-muted/50 rounded-xl h-14">
                    <TabsTrigger value="scheduled" className="rounded-lg h-12 font-medium data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
                        Upcoming <span className="ml-2 bg-primary/10 text-primary px-2 py-0.5 rounded-full text-xs">{upcomingAppointments.length}</span>
                    </TabsTrigger>
                    ...
                    <TabsTrigger value="history" className="rounded-lg h-12 font-medium data-[state=active]:bg-white data-[state=active]:text-muted-foreground data-[state=active]:shadow-sm transition-all">
                        History <span className="ml-2 bg-muted text-muted-foreground px-2 py-0.5 rounded-full text-xs">{historyAppointments.length}</span>
                    </TabsTrigger>
                </TabsList>
                ...
                <div className="mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <TabsContent value="pending" className="space-y-4">
                        {pendingAppointments.length > 0 ? (
                            pendingAppointments.map(app => <AppointmentCard key={app.id} appointment={app} showCancel={true} />)
                        ) : (
                            <EmptyState message="No pending appointment requests." icon={Clock} />
                        )}
                    </TabsContent>

                    <TabsContent value="scheduled" className="space-y-4">
                        {upcomingAppointments.length > 0 ? (
                            upcomingAppointments.map(app => <AppointmentCard key={app.id} appointment={app} showCancel={true} />)
                        ) : (
                            <EmptyState message="No upcoming appointments scheduled." icon={Calendar} />
                        )}
                    </TabsContent>

                    <TabsContent value="history" className="space-y-4">
                        {historyAppointments.length > 0 ? (
                            historyAppointments.map(app => <AppointmentCard key={app.id} appointment={app} showDelete={true} showRate={true} />)
                        ) : (
                            <EmptyState message="No past appointment history." icon={User} />
                        )}
                    </TabsContent>
                </div>
            </Tabs>
