...
import { User, Star, Stethoscope, Brain, Search, CalendarPlus } from "lucide-react";
  ...
  const filteredProviders = useMemo(() => {
    ...
    return providers.filter((provider) => {
      ...
      return (
        ...
        (profile.specialization || profile.expertise || "").toLowerCase().includes(lowerQuery)
      );
    });
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-muted-foreground font-medium">Loading available providers...</p>
        </div>
      </div>
    );
  }
          ...
          <CardHeader>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <CardTitle className="text-2xl font-bold">Find a Provider</CardTitle>
...
const ProviderCard: React.FC<{ provider: Provider }> = ({ provider }) => {
  ...
  const specialization = "specialization" in profile ? profile.specialization : profile.expertise;
      ...
      <CardHeader className="items-center text-center pb-4 bg-muted/10 border-b border-border/40">
        <div className="relative">
            ...
            <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2">
                <Badge variant="secondary" className="text-[10px] px-2 py-0.5 font-semibold shadow-sm border-white border flex items-center gap-1 whitespace-nowrap">
        ...
        <div className="mt-3">
            <CardTitle className="text-lg font-bold line-clamp-1">{profile.full_name}</CardTitle>
      ...
      <CardContent className="text-center pt-6 pb-6 px-6 flex-grow flex flex-col justify-between gap-4">
        ...
        <div className="space-y-4 mt-auto">
          <div className="flex items-center justify-center gap-1.5 bg-yellow-50 dark:bg-yellow-900/10 py-1.5 px-3 rounded-full w-fit mx-auto">
            ...
            <span className="font-bold text-sm text-yellow-700 dark:text-yellow-500">{profile.rating || "5.0"}</span>
            <span className="text-xs text-yellow-600/70 dark:text-yellow-500/70 font-medium">Rating</span>
          </div>

          <Button asChild className="w-full shadow-sm group-hover:shadow-md transition-all">
