...
import BookingForm from '@/components/appointments/BookingForm'; // Import the new form
// You need to install date-fns: npm install date-fns
import { format, addDays, startOfWeek, endOfWeek, isSameDay, isBefore, startOfToday } from 'date-fns';
...
const ProviderDetailPage: React.FC = () => {
    ...
    const { data: availability, isLoading: isLoadingAvailability } = useQuery<AvailabilityData>({
        ...
        queryFn: () => fetchAvailability(providerId!, weekStart, startOfWeek(addDays(weekStart, 13), {weekStartsOn: 1})), // Fetch 2 weeks at a time
        enabled: !!providerId,
    });
    ...
    if (isLoadingProvider || !provider) return <p className="p-6">Loading provider profile...</p>;
    ...
    return (
        <div className="p-6 max-w-5xl mx-auto">
            ...
            <Card>
                ...
                <CardContent>
                    {isLoadingAvailability ? <p className="text-center">Loading availability...</p> : (
                            ...
                            {days.map(day => {
                                ...
                                return (
                                    <div key={day.toISOString()} className="text-center">
                                        ...
                                        <p className={`text-sm ${isSameDay(day, today) ? 'text-primary font-bold' : 'text-muted-foreground'}`}>{format(day, 'd')}</p>
                                        <div className="space-y-2 mt-2">
                                            {isPast ? <div className="text-xs text-muted-foreground h-8 flex items-center justify-center">Past</div> : daySlots.length > 0 ? (
                                                daySlots.map(time => (
                                                    <Button key={time} variant="outline" className="w-full" onClick={() => handleTimeSlotClick(day, time)}>
                                                        {time}
                                                    </Button>
                                                ))
                                            ) : <div className="text-xs text-muted-foreground h-8 flex items-center justify-center">Unavailable</div>}
                                        </div>
                                    </div>
                                );
                            })}
